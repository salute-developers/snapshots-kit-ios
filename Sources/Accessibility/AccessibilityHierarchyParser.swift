...
final class AccessibilityHierarchyParser {
    ...
    func parseAccessibilityElements(
        ...
        return accessibilityElements.map { element in
            ...
            let accessibilityIdentifierSelector = Selector(("accessibilityIdentifier"))
            if element.object.responds(to: accessibilityIdentifierSelector) {
                let unmanaged = element.object.perform(accessibilityIdentifierSelector)
                accessibilityIdentifier = unmanaged?.takeUnretainedValue() as? String
            } else {
            ...
            return AccessibilityMarker(
                ...
                customActions: element.object.accessibilityCustomActions?.map(\.name) ?? [],
    ...
    private func accessibilityShape(for element: NSObject, in root: UIView) -> AccessibilityMarker.Shape {
        if let element = element as? UIAccessibilityElement, let container = element.accessibilityContainer, !element.accessibilityFrameInContainerSpace.isNull {
...
extension NSObject {
    ...
    /// them.
    @MainActor
    fileprivate func recursiveAccessibilityHierarchy(
...
extension CGPoint {
    fileprivate func approximatelyEquals(_ other: CGPoint, tolerance: CGFloat) -> Bool {
