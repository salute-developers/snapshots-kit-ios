...
class SnapshotAndLegendView: UIView {
    ...
    private var minimumWidth: CGFloat {
        minimumLegendWidth + Metrics.legendInsets.left + Metrics.legendInsets.right
    }
    ...
    private func legendLocation(viewSize: CGSize) -> LegendLocation {
        ...
        if aspectRatio > 1 || viewSize.width < minimumWidth {
            ...
            // Tall views that meet the minimum height requirement
            // should display the legend to the right of the snapshotted view.
...
extension CGFloat {
    @MainActor
    fileprivate func floorToPixel(in source: UIWindow?) -> CGFloat {
        let scale = source?.screen.scale ?? 1
        return floor(self * scale) / scale
    }

    @MainActor
    fileprivate func ceilToPixel(in source: UIWindow?) -> CGFloat {
        let scale = source?.screen.scale ?? 1
